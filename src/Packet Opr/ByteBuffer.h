...
public:
	...
	ByteBuffer(const ByteBuffer& buf) : m_doubleByte(false), _rpos(buf._rpos), _wpos(buf._wpos), _storage(buf._storage)
	{
	}
	...
	{
		if (_rpos + len <= size())
			CopyMemory(dest, &_storage[_rpos], len);
		else // throw error();
			memset(dest, 0, len);
		_rpos += len;
	};
	...
	void append(const void* src, size_t cnt)
	{
		...
		CopyMemory(&_storage[_wpos], src, cnt);
		_wpos += cnt;
	}
	...
	{
		ASSERT(pos + cnt <= size());
		CopyMemory(&_storage[pos], src, cnt);
	}
