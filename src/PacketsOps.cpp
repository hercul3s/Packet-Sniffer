...
void DecryptAndPrintOpr(BYTE* m_Data, SYSTEMTIME currentTime, std::string srcStr)
{
	...
	if (m_Data[0] == WIZ_VERSION_CHECK && m_Data[1] == 0xFF && m_DataLen == 0x03)
	{
		isLsCryption = FALSE;
		isGsCryption = FALSE;
		PrintPacketTitle(m_DataLen, srcStr, currentTime);//hERE
		PrintPacket(m_Data, m_DataLen);
		return;
	}
	...
	{
		const std::unique_ptr<BYTE[]> tmpData(new BYTE[m_DataLen]);
		CopyMemory(tmpData.get(), m_Data, m_DataLen);
		
		AES_Encryption.Cryption(tmpData.get(), m_DataLen, false, tmpData.get());
		if (m_DataLen > 500)
		{
			if (tmpData[0] == WIZ_COMPRESS_PACKET)
			{
				uint32 inLen, outLen;
				CopyMemory(&inLen, &tmpData[1], 4);
				CopyMemory(&outLen, &tmpData[5], 4);
				const std::unique_ptr<BYTE[]> inData(new BYTE[MAX_BUFFERSIZE]);
				const std::unique_ptr<BYTE[]> outData(new BYTE[outLen + (BUFFERSIZETOLERANCE)]);
				CopyMemory(inData.get(), &tmpData[13], inLen);

				const int pSize = lzf_decompress(inData.get(), inLen, outData.get(), outLen);
				PrintPacketTitle(pSize, srcStr, currentTime);
				/*PrintPacket(outData.get(), pSize);*/
				return;
			}
		...
		PrintPacketTitle(m_DataLen, srcStr, currentTime);
		PrintPacket(tmpData.get(), m_DataLen);
		return;
	}
...
void PacketsOps::DataCheckAndMerge(BYTE* m_Data, int m_DataLen, std::string srcStr)
{
	...
	if (HeaderCheck(m_Data))
	{
		...
		mergedPacketBuffer = std::unique_ptr < BYTE[]>(new BYTE[MERGEDPACKETBUFFERSIZE * sizeof(BYTE)]) ;
	...
	if (FooterCheck(m_Data, m_DataLen))
	{
		...
		DecryptAndPrintOpr(mergedPacketBuffer.get(), currentTime, srcStr);
		mergedPacketSize = NULL;		
		//mergedPacketBuffer.reset();
		//free(mergedPacketBuffer.get());
